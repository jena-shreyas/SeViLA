...
sevila.load_checkpoint(url_or_filename='./sevila_checkpoints/sevila_pretrained.pth')
print('Model Loaded')
...
while True:
    video_name = input("\nVideo name : ") # Format : *.mp4
    vpath = os.path.join(dir_path, video_name)
    question = input('Question : ') # 'Why did the two ladies put their hands above their eyes while staring out?'
    if question == '-1':
        break
    option1 = input('Option 1 : ') # 'practicing cheer'
    option2 = input('Option 2 : ') # 'to place wreaths'
    option3 = input('Option 3 : ') # 'to see better'
    video_frame_num = 32
    keyframe_num = 4

    keyframes, timestamps_des, answer = sevila_demo(sevila, vpath, question, \
                            option1, option2, option3, \
                            video_frame_num, keyframe_num)
